...
export class NCCAPIService {
    ...
    createAutomaticNote(call_id: string, ticket_number: string, tenancy_reference: string, call_reason_id: string, crm_contact_id: string,
        content: string, parameters: { [propKey: string]: any } = null) {
        return this._createNote(call_id, ticket_number, call_reason_id, crm_contact_id, content, true, parameters);
    }
    ...
    _createNote(call_id: string, ticket_number: string, call_reason_id: string, crm_contact_id: string, content: string,
        automatic: boolean = false, additional_parameters: { [propKey: string]: any } = null) {

        // TODO pass an object to extend the parameters.

        // "Other" is generated by this app, and doesn't exist in the CRM as a call reason.
        if (CALL_REASON.OTHER === call_reason_id) {
            call_reason_id = null;
        }
    ...
    private _redirectToParis(interaction_id: string, amount: number) {
        ...
        const return_data = [
            ...
            interaction_id,
            '327F0B75-11D9-E811-A96E-002248072CC3', // sshetty
            amount
        ];
